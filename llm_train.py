...
try:
    ...
    logger.error("\n请检查 CSV 文件中的列名是否正确：")
    logger.error("train.csv 应包含 'Id', 'Sentence', 'Category' 列。")
    logger.error("test.csv 应包含 'Id', 'Sentence' 列。")
    if train_df is not None: logger.info(f"train.csv 实际列名: {train_df.columns.tolist()}")
    if test_df is not None: logger.info(f"test.csv 实际列名: {test_df.columns.tolist()}")
    exit(1)
...
if len(train_dataset) > 0:
    ...
    logger.info(f"原训练集分割完成：")
    logger.info(f"  训练集大小: {len(train_dataset_split)}")
    logger.info(f"  验证集大小: {len(eval_dataset_split)}")
else:
    logger.error("错误：训练数据集为空，无法进行分割。")
    exit(1)

logger.info("\n--- 3. 加载模型和分词器 (从本地) ---")

logger.info(f"正在尝试从本地加载模型和 Tokenizer: {local_model_path}")
...
logger.info("4-bit 量化配置已设置。")

try:
    tokenizer = AutoTokenizer.from_pretrained(local_model_path, trust_remote_code=True)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
        logger.info("Tokenizer pad_token 已设置为 eos_token。")
    logger.info("Tokenizer 加载完成！")
except Exception as e:
    logger.error(f"从本地加载 Tokenizer 时发生错误: {e}")
    logger.error(f"请检查本地模型路径 '{local_model_path}' 是否包含正确的 tokenizer 文件。")
    exit(1)
...
try:
    ...
    logger.info(f"模型已在 {model.device} 上加载 (4-bit 量化)。")
except Exception as e:
    logger.error(f"从本地加载模型时发生错误: {e}")
    logger.error(f"请检查本地模型路径 '{local_model_path}' 是否包含正确的模型文件。")
    exit(1)
...
logger.info(f"类别映射: {category_to_id}")
...
logger.info("正在分析模型结构以找到正确的目标模块...")
target_modules = []
...
logger.info(f"可用的目标模块: {TARGET_MODULES}")
...
logger.info("准备模型进行 4-bit 训练...")
model = prepare_model_for_kbit_training(model, use_gradient_checkpointing=False)

logger.info("将 LoRA 适配器应用到模型...")
...
logger.info("训练参数定义完成。")

from transformers import DataCollatorForLanguageModeling

data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=False)
logger.info("DataCollator 准备完成。")
